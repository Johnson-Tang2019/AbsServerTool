...
public final class SnapshotScheduler {
    ...
    private static void tick(MinecraftServer server) {
        ...
        if (!today.equals(lastObservedDate)) { SnapshotStore.captureDayBaseline(server, today, false); SnapshotStore.cleanupOldSnapshots(server, today); lastObservedDate = today; AbsServerTool.LOGGER.info("Captured daily vanilla statistics baseline {}", today); }
    }
}
